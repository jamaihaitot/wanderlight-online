...
namespace WanderlightOnline.Tests.Contract
{
    ...
    public class PlayerManagerContractTests
    {
        ...
        {
            // Add a player
            var result = _playerManager.TryAddPlayer("SpawnTestPlayer", out var error);
            AssertThat(result).IsTrue();
            AssertThat(error).IsNull();

            // Get the player and verify it exists
            var player = _playerManager.GetPlayer("SpawnTestPlayer");
        ...
        public void PlayerStateRestoredOnReconnect()
        {
            ...
            // First, add a player
            var result = _playerManager.TryAddPlayer("StateTestPlayer", out var error);
            AssertThat(result).IsTrue();

            var player = _playerManager.GetPlayer("StateTestPlayer");
            AssertThat(player).IsNotNull();
            ...
            // Simulate disconnect
            _playerManager.RemovePlayer("StateTestPlayer");
            AssertThat(_playerManager.GetPlayer("StateTestPlayer")).IsNull();
        ...
        public void InventoryActionsAreAtomic()
        {
            ...
            var result = _playerManager.TryAddPlayer("AtomicTestPlayer", out var error);
            AssertThat(result).IsTrue();

            var player = _playerManager.GetPlayer("AtomicTestPlayer");
            AssertThat(player).IsNotNull();
